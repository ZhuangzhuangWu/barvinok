	...
	__isl_take isl_multi_union_pw_aff *mupa)
{
	...
	/* polyacc Extended */
	band->space_time = isl_calloc_array(ctx, enum isl_polyacc_loop_type, band->n);
  	band->pe_opt = isl_calloc_array(ctx, enum isl_polyacc_loop_type, band->n);
	...
	__isl_keep isl_schedule_band *band)
{
	...
	/* polyacc Extended */
    if (band->space_time) {
      dup->space_time = isl_alloc_array(ctx, enum isl_polyacc_loop_type, band->n);
      for (i = 0; i < band->n; ++i)
        dup->space_time[i] = band->space_time[i];
    }
    if (band->pe_opt) {
      dup->pe_opt = isl_alloc_array(ctx, enum isl_polyacc_loop_type, band->n);
      for (i = 0; i < band->n; ++i)
 ...
 * the band.
 */
enum isl_polyacc_loop_type isl_schedule_band_member_get_space_time(
  __isl_keep isl_schedule_band *band, int pos)
{
  if (!band)
  ...
  if (pos < 0 || pos >= band->n)
    isl_die(isl_schedule_band_get_ctx(band), isl_error_invalid,
        "invalid member position", return isl_polyacc_loop_error);
...
__isl_give isl_schedule_band *isl_schedule_band_member_set_space_time(
  __isl_take isl_schedule_band *band, int pos, enum isl_polyacc_loop_type loop_type)
{
  if (!band)
    return NULL;
  band = isl_schedule_band_cow(band);
  ...
  if (!band->space_time)
    band->space_time = isl_calloc_array(isl_schedule_band_get_ctx(band), 
			enum isl_polyacc_loop_type, band->n);
 ...
 * the band.
 */
enum isl_polyacc_loop_type isl_schedule_band_member_get_pe_opt(
  __isl_keep isl_schedule_band *band, int pos)
{
  if (!band)
    return isl_polyacc_loop_error;
