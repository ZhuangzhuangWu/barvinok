...
static isl_bool any_space_time(__isl_keep isl_schedule_band *band)
{
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (space_time == isl_polyacc_loop_time || space_time == isl_polyacc_loop_spatial_x 
		|| space_time == isl_polyacc_loop_spatial_y || space_time == isl_polyacc_loop_spatial_z)
      return isl_bool_true;
  }
...
static isl_bool any_pe_opt(__isl_keep isl_schedule_band *band)
{
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (pe_opt == isl_polyacc_loop_latency || pe_opt == isl_polyacc_loop_simd || 
				pe_opt == isl_polyacc_loop_array_part)
      return isl_bool_true;
  }
  ...
  if (space_time) {
    ...
    for (i = 0; i < n; ++i) {
      switch(isl_schedule_band_member_get_space_time(band, i)) {
        case isl_polyacc_loop_default:
          p = isl_printer_print_str(p, "default");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_error:
          p = isl_printer_print_str(p, "error");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_time:
          p = isl_printer_print_str(p, "time");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_spatial_x:
          p = isl_printer_print_str(p, "Spatial-X");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_spatial_y:
          p = isl_printer_print_str(p, "Spatial-Y");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_spatial_z:
          p = isl_printer_print_str(p, "Spatial-Z");
          p = isl_printer_yaml_next(p);
          break; 
        default:
    ...
    for (i = 0; i < n; ++i) {
      switch(isl_schedule_band_member_get_pe_opt(band, i)) {
        ...
        case isl_polyacc_loop_error:
          ...
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_latency:
          p = isl_printer_print_str(p, "latency");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_simd:
          p = isl_printer_print_str(p, "simd");
          p = isl_printer_yaml_next(p);
          break;
        case isl_polyacc_loop_array_part:
 ...
 */
enum isl_polyacc_loop_type isl_schedule_tree_band_member_get_space_time(
  __isl_keep isl_schedule_tree *tree, int pos)
{
  if (!tree)
  ...
  if (tree->type != isl_schedule_node_band)
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_invalid,
        "not a band node", return isl_polyacc_loop_error);
...
__isl_give isl_schedule_tree *isl_schedule_tree_band_member_set_space_time(
  __isl_take isl_schedule_tree *tree, int pos, enum isl_polyacc_loop_type loop_type)
{
  if (!tree)
    return NULL;
  if (tree->type != isl_schedule_node_band)
 ...
 * "pos" of the band tree root.
 */
enum isl_polyacc_loop_type isl_schedule_tree_band_member_get_pe_opt(
  __isl_keep isl_schedule_tree *tree, int pos)
{
  if (!tree)
    return isl_size_error;
...
// struct tree_band_prop *extract_tree_band_prop(
// 	__isl_keep isl_schedule_tree *tree)
// {
//   struct tree_band_prop *prop = isl_calloc_type(
//       isl_schedule_tree_get_ctx(tree), struct tree_band_prop);
//   prop->mupa = isl_schedule_tree_band_get_partial_schedule(tree->band);
//   prop->n_member = isl_schedule_tree_band_n_member(tree);
//   prop->coincident = isl_calloc_array(isl_schedule_tree_get_ctx(tree), int,
//                                       prop->n_member);
//   for (int i = 0; i < prop->n_member; i++)
//   {
//     prop->coincident[i] = isl_schedule_tree_band_member_get_coincident(tree, i);
//   }
//   prop->permutable = isl_schedule_tree_band_get_permutable(tree);
//   prop->space_time = isl_calloc_array(isl_schedule_tree_get_ctx(tree),
//                                       enum isl_polyacc_loop_type, prop->n_member);
//   prop->pe_opt = isl_calloc_array(isl_schedule_tree_get_ctx(tree),
//                                   enum isl_polyacc_loop_type, prop->n_member);
//   prop->sched_pos = isl_calloc_array(isl_schedule_tree_get_ctx(tree),
//                                      int, prop->n_member);  
//   for (int i = 0; i < prop->n_member; i++)
//   {
//     prop->space_time[i] = isl_schedule_tree_band_member_get_space_time(tree, i);
//     prop->pe_opt[i] = isl_schedule_tree_band_member_get_pe_opt(tree, i);
//     prop->sched_pos[i] = isl_schedule_tree_band_member_get_sched_pos(tree, i);
//     prop->iter[i] = isl_schedule_tree_band_member_get_iter(tree, i);
//   }  

//   return prop;
// }

// __isl_give isl_schedule_tree *isl_schedule_tree_merge(
// 	__isl_take isl_schedule_tree *tree)
// {
// 	printf("[debug] isl_schedule_tree_merge initial.\n");
// 	isl_schedule_tree *parent = tree;
// 	isl_schedule_tree *child = isl_schedule_tree_child(tree, 0);

// 	printf("[debug] Save the tree properties \n");
// 	/* Save the tree properties. */
// 	struct tree_band_prop *parent_prop = extract_tree_band_prop(parent);
// 	struct tree_band_prop *child_prop = extract_tree_band_prop(child);

// 	printf("[debug] Save the tree properties done. \n");
// 	printf("[debug] Begin to merge the partial schedules of two trees. \n");


// 	/* Merge the partial schedules of two trees. */
// 	isl_union_pw_aff_list *upa_list = isl_union_pw_aff_list_alloc(
// 		isl_schedule_tree_get_ctx(tree), 0);
...
// 		upa_list = isl_union_pw_aff_list_add(upa_list, upa);
// 	}
// 	printf("[debug] begin to merge the multi_union_pw_aff of two trees. \n");
...
// 	printf("[debug] Merge the multi_union_pw_aff of two trees done. \n");
// 	printf("[debug] Begin to insert one new tree. \n");
...
// 	printf("[debug] Insert one new tree done. \n");
// 	printf("[debug] Begin to restore the tree properties. \n");

// 	/* Restore the tree properties. */
// 	tree = isl_schedule_tree_band_set_permutable(tree, 1);
// 	for (int i = 0; i < parent_prop->n_member; i++)
// 	{
// 		tree = isl_schedule_tree_band_member_set_coincident(tree, i,
// 			parent_prop->coincident[i]);
// 	}
// 	for (int i = 0; i < parent_prop->n_member; i++)
// 	{
// 		tree = isl_schedule_tree_band_member_set_space_time(tree, i,
// 			parent_prop->space_time[i]);
// 		tree = isl_schedule_tree_band_member_set_pe_opt(tree, i,
// 			parent_prop->pe_opt[i]);
// 		tree = isl_schedule_tree_band_member_set_sched_pos(tree, i,
// 			parent_prop->sched_pos[i]);
// 		tree = isl_schedule_tree_band_member_set_iter(tree, i,
// 			parent_prop->iter[i]);
// 	}
// 	for (int i = 0; i < child_prop->n_member; i++)
// 	{
// 		tree = isl_schedule_tree_band_member_set_coincident(tree, i + parent_prop->n_member, child_prop->coincident[i]);
// 	}
// 	for (int i = 0; i < child_prop->n_member; i++)
// 	{
// 		tree = isl_schedule_tree_band_member_set_space_time(tree, i + parent_prop->n_member, child_prop->space_time[i]);
// 		tree = isl_schedule_tree_band_member_set_pe_opt(tree, i + parent_prop->n_member, child_prop->pe_opt[i]);
// 		tree = isl_schedule_tree_band_member_set_sched_pos(tree, i + parent_prop->n_member, child_prop->sched_pos[i]);
// 		tree = isl_schedule_tree_band_member_set_iter(tree, i + parent_prop->n_member, child_prop->iter[i]);
// 	}
...
// 	printf("[debug] Restore the tree properties done. \n");
...
// 	printf("[debug] Merge the partial schedules of two trees done. \n");
