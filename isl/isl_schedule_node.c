 ...
 * band node "node".
 */
enum isl_polyacc_loop_type isl_schedule_node_band_member_get_space_time(
	__isl_keep isl_schedule_node *node, int pos)
{
	if (!node)
		return isl_polyacc_loop_error;
...
__isl_give isl_schedule_node *isl_schedule_node_band_member_set_space_time(
	__isl_take isl_schedule_node *node, int pos, enum isl_polyacc_loop_type loop_type)
{
	enum isl_polyacc_loop_type t;
	isl_schedule_tree *tree;
 ...
 * band node "node".
 */
enum isl_polyacc_loop_type isl_schedule_node_band_member_get_pe_opt(
	__isl_keep isl_schedule_node *node, int pos)
{
	if (!node)
		return isl_polyacc_loop_error;
...
struct node_band_prop
{
	isl_bool permutable;
	isl_bool *coincident;
	enum isl_polyacc_loop_type *pe_opt;
	enum isl_polyacc_loop_type *space_time;
	int *sched_pos;
	void *iter[20];
	isl_size n_member;
	isl_multi_union_pw_aff *mupa;
...
struct node_band_prop *extract_node_band_prop(
	__isl_keep isl_schedule_node *node)
{
	struct node_band_prop *prop = isl_calloc_type(
		isl_schedule_node_get_ctx(node), struct node_band_prop);
	...
	prop->space_time = isl_calloc_array(isl_schedule_node_get_ctx(node),
										enum isl_polyacc_loop_type, prop->n_member);
 ...
 */
__isl_give isl_schedule_node *isl_schedule_node_merge(
	__isl_take isl_schedule_node *node)
{
	...
	// printf("[debug] isl_schedule_node_merge initial node: \n");
	// isl_schedule_node_dump(node);

	// printf("[debug] isl_schedule_node_merge begin to merge\n");

	// isl_schedule_node *parent = node;
	// isl_schedule_node *child = isl_schedule_node_child(node, 0);

	// printf("[debug] Save the node properties \n");
	/* Save the node properties. */
	struct node_band_prop *parent_prop = extract_node_band_prop(parent);
	struct node_band_prop *child_prop = extract_node_band_prop(child);

	// printf("[debug] Save the node properties done. \n");
	// printf("[debug] Begin to merge the partial schedules of two nodes. \n");
	...
	for (int i = 0; i < parent_prop->n_member; i++)
	{
		...
		upa_list = isl_union_pw_aff_list_add(upa_list, upa);
	}
	...
	// printf("[debug] begin to merge the multi_union_pw_aff of two nodes. \n");
	...
	isl_multi_union_pw_aff *mupa = isl_multi_union_pw_aff_from_union_pw_aff_list(mupa_space, upa_list);
	// printf("[debug] Merge the multi_union_pw_aff of two nodes done. \n");
	// printf("[debug] Begin to insert one new node. \n");

	/* Insert one new node. */
	node = isl_schedule_node_insert_partial_schedule(node, mupa);

	// printf("[debug] Insert one new node done. \n");
	// printf("[debug] Begin to restore the node properties. \n");
	...
	for (int i = 0; i < parent_prop->n_member; i++)
	{
		node = isl_schedule_node_band_member_set_coincident(node, i,
															parent_prop->coincident[i]);
	}
	for (int i = 0; i < parent_prop->n_member; i++)
	{
		node = isl_schedule_node_band_member_set_space_time(node, i,
															parent_prop->space_time[i]);
		node = isl_schedule_node_band_member_set_pe_opt(node, i,
														parent_prop->pe_opt[i]);
		node = isl_schedule_node_band_member_set_sched_pos(node, i,
														   parent_prop->sched_pos[i]);
		node = isl_schedule_node_band_member_set_iter(node, i,
													  parent_prop->iter[i]);
	}
	for (int i = 0; i < child_prop->n_member; i++)
	{
	...
	for (int i = 0; i < child_prop->n_member; i++)
	{
		...
		node = isl_schedule_node_band_member_set_sched_pos(node, i + parent_prop->n_member, child_prop->sched_pos[i]);
		node = isl_schedule_node_band_member_set_iter(node, i + parent_prop->n_member, child_prop->iter[i]);
	}
	// printf("[debug] Merge the partial schedules of two nodes done. \n");
	...
	node = isl_schedule_node_delete(node);
	node = isl_schedule_node_parent(node);
	// printf("[debug] Restore the node properties done. \n");
	...
	// printf("[debug] isl_schedule_node_merge merge done \n");

	// printf("[debug] isl_schedule_node_merge finial node\n");
	// isl_schedule_node_dump(node);

	return node;
}

#define DBGUPA(os, upa, ctx, str)                                   \
	{                                                               \
...
#define DBGPA(os, pa, ctx, str)                               \
	{                                                         \
		printf("%s(%d) Print pw_aff.\n", __FILE__, __LINE__); \
		printf("%s\n", str);                                  \
		isl_printer *p_debug = isl_printer_to_file(ctx, os);  \
		p_debug = isl_printer_print_pw_aff(p_debug, pa);      \
		p_debug = isl_printer_print_str(p_debug, "\n");       \
		p_debug = isl_printer_free(p_debug);                  \
	}

#define DBGVAL(os, val, ctx, str)                            \
	{                                                        \
...
__isl_give isl_multi_union_pw_aff *isl_schedule_node_get_prefix_time_multi_union_pw_aff(
	__isl_keep isl_schedule_node *node, __isl_keep isl_multi_union_pw_aff *spatial_mupa, __isl_take isl_mat *dep_mat)
{
	enum isl_schedule_node_type type;
	enum isl_polyacc_loop_type loop_type;

	isl_ctx *ctx = isl_schedule_node_get_ctx(node);
	isl_space *node_space = isl_space_set_alloc(isl_schedule_node_get_ctx(node), 0, 0);
	isl_union_pw_aff_list *upa_list = isl_union_pw_aff_list_alloc(isl_schedule_node_get_ctx(node), 0);
	isl_size n_member;
	while (isl_schedule_node_has_parent(node))
	{
		...
		if (type == isl_schedule_node_band)
		{
			...
			isl_multi_union_pw_aff *band_mupa;
			band_mupa = isl_schedule_node_band_get_partial_schedule(node);
			for (int i = n_member - 1; i >= 0; i--)
			{
				loop_type = isl_schedule_node_band_member_get_space_time(node, i);
				if (loop_type == isl_polyacc_loop_time)
				{
					node_space = isl_space_add_dims(node_space, isl_dim_set, 1);
					isl_union_pw_aff *upa = isl_multi_union_pw_aff_get_union_pw_aff(band_mupa, i);
					upa_list = isl_union_pw_aff_list_add(upa_list, upa);
				}
			}
			// node = isl_schedule_node_parent(node);
	...
	isl_union_pw_aff_list *upa_list_reverse;
	isl_size space_loop_dim, rank;
	isl_union_pw_aff *spatial_upa;
	isl_union_pw_aff *sum_upa;
	isl_vec *dep_vec;
	isl_val *sum;
	isl_multi_union_pw_aff *mupa;

	upa_list_reverse = isl_union_pw_aff_list_reverse(upa_list);

	// mupa = isl_multi_union_pw_aff_from_union_pw_aff_list(node_space, upa_list_reverse);

	space_loop_dim = isl_multi_union_pw_aff_dim(spatial_mupa, isl_dim_out);
	rank = isl_mat_rank(dep_mat);
	dep_vec = isl_vec_zero(ctx, isl_mat_cols(dep_mat));
	for (int col = 0; col < isl_mat_cols(dep_mat); col++)
	{
		sum = isl_val_zero(ctx);
		for (int row = 0; row < isl_mat_rows(dep_mat); row++)
		{
			sum = isl_val_add(isl_val_copy(sum), isl_val_copy(isl_mat_get_element_val(dep_mat, row, col)));
		}
		// isl_val_dump(sum);
		if (!isl_val_is_zero(sum))
		{
			dep_vec = isl_vec_set_element_val(dep_vec, col, isl_val_one(ctx));
		}
	}
	if (rank == 0)
	{
		// printf("[INFO] No interconnection between units\n");
		if (isl_union_pw_aff_list_n_union_pw_aff(upa_list_reverse) == 0)
		{
			isl_pw_aff *pwaff_0;
			isl_set *set_0;
			isl_union_pw_aff *upa_0;
			isl_union_set *domain;

			domain = isl_schedule_node_get_domain(node);
			set_0 = isl_set_from_union_set(domain);
			pwaff_0 = isl_pw_aff_val_on_domain(set_0, isl_val_zero(ctx));
			sum_upa = isl_union_pw_aff_from_pw_aff(pwaff_0);
			mupa = isl_multi_union_pw_aff_from_union_pw_aff(sum_upa);
		}
		else{
			mupa = isl_multi_union_pw_aff_from_union_pw_aff_list(node_space, upa_list_reverse);
		}
	...
	{
		// printf("[INFO] the rank of interconnect is %d\n", rank);
		if (rank == isl_multi_union_pw_aff_dim(spatial_mupa, isl_dim_out))
		{
			for (int upa_size = 0; upa_size < isl_union_pw_aff_list_n_union_pw_aff(upa_list_reverse); upa_size++)
			{
				if (upa_size == 0)
				{
					sum_upa = isl_union_pw_aff_list_get_union_pw_aff(upa_list_reverse, 0);
				}
				else
				{
					sum_upa = isl_union_pw_aff_add(sum_upa, isl_union_pw_aff_list_get_union_pw_aff(upa_list_reverse, upa_size));
				}
			}
			// printf("rank == isl_din_out time sum_upa:\n");
			// isl_union_pw_aff_dump(sum_upa);
			if (isl_union_pw_aff_list_n_union_pw_aff(upa_list_reverse) == 0)
			{
				isl_pw_aff *pwaff_0;
				isl_set *set_0;
			...
			for (int size = 0; size < rank; size++)
			{
				spatial_upa = isl_multi_union_pw_aff_get_union_pw_aff(spatial_mupa, size);
				// printf("spatial_upa:\n");
				// isl_union_pw_aff_dump(spatial_upa);
				// if (isl_union_pw_aff_list_n_union_pw_aff(upa_list_reverse) == 0 && size == 0)
				// {
				// 	sum_upa = isl_union_pw_aff_copy(spatial_upa);
				// }
				// else
				// {
				sum_upa = isl_union_pw_aff_add(sum_upa, isl_union_pw_aff_copy(spatial_upa));
				// }
				// printf("for_loop sum_upa:\n");
				// isl_union_pw_aff_dump(sum_upa);
			}
			mupa = isl_multi_union_pw_aff_from_union_pw_aff(sum_upa);
		}
		else
		{
			...
			// printf("rank != isl_din_out time sum_upa:\n");
			// isl_union_pw_aff_dump(sum_upa);
			for (int size = 0; size < isl_vec_size(dep_vec); size++)
			{
				if (isl_val_is_one(isl_vec_get_element_val(dep_vec, size)))
				{
					spatial_upa = isl_multi_union_pw_aff_get_union_pw_aff(spatial_mupa, size);
					// printf("spatial_upa:\n");
					// isl_union_pw_aff_dump(spatial_upa);
...
{
	isl_union_pw_aff *inter_dim;
	isl_union_pw_aff *result;

	//"sum" is the total number of union_pw_aff max bound
	isl_val *sum;

	int inter_dim_valid;
};
/* Free the intermediate_data struct. */
void *intermediate_data_free(struct intermediate_data *dim)
{
	if (!dim)
		return NULL;

	isl_union_pw_aff_free(dim->inter_dim);
	isl_union_pw_aff_free(dim->result);
	isl_val_free(dim->sum);
	free(dim);
	return NULL;
}

// Obtain the space dimension size of input "domain".
static isl_val *obatian_union_pw_aff_bound(
	__isl_take isl_union_pw_aff *union_pw_aff, __isl_take isl_union_set *domain)
{

	isl_val *ub;
	isl_union_pw_aff *upa_domain;
	upa_domain = isl_union_pw_aff_intersect_domain(union_pw_aff, domain);
	ub = isl_union_pw_aff_max_val(upa_domain);
	return ub;
}

// Obtain empty pw_aff
// s[i,j,k]->[0]
static isl_union_pw_aff *get_empty_union_pw_aff(__isl_keep isl_union_pw_aff *union_pw_aff)
{

	isl_ctx *ctx;
	isl_val *zero;
	isl_union_pw_aff *empty;

	ctx = isl_union_pw_aff_get_ctx(union_pw_aff);
	zero = isl_val_zero(ctx);
	empty = isl_union_pw_aff_scale_val(union_pw_aff, zero);
	return empty;
}
struct intermediate_data *get_dimission_union_pw_aff(
	isl_ctx *ctx, __isl_keep isl_union_pw_aff_list *upa_list, __isl_keep isl_union_set *domain, isl_val *grid_size)
{

	// isl_size upa_list_size = isl_union_pw_aff_list_size(isl_union_pw_aff_list_copy(upa_list));
	// if(upa_list <= 0){
	// 	return NULL;
	// }
	// printf("[Function] get_dimission_union_pw_aff: enter func.");
	struct intermediate_data *dim = isl_calloc_type(ctx, struct intermediate_data);

	int i;
	isl_size n_upa;
	isl_union_pw_aff *upa;
	isl_union_pw_aff *tmp;
	isl_val *previous_bound;
	isl_val *ub;
	isl_val *sum_ub;
	isl_val *one;

	dim->inter_dim_valid = 0;
	n_upa = isl_union_pw_aff_list_n_union_pw_aff(upa_list);
	isl_union_pw_aff_list *upa_list_reverse = isl_union_pw_aff_list_reverse(isl_union_pw_aff_list_copy(upa_list));
	// printf("[Function] get_dimission_union_pw_aff: upa_list_reverse\n");
	// isl_union_pw_aff_list_dump(upa_list_reverse);
	// printf("[Function] get_dimission_union_pw_aff: n_upa=%d.\n", n_upa);
	one = isl_val_one(ctx);
	previous_bound = isl_val_copy(one);
	// printf("[Function] get_dimission_union_pw_aff: loop start.\n");
	for (i = 0; i < n_upa; i++)
	{
		upa = isl_union_pw_aff_list_get_union_pw_aff(upa_list_reverse, i);
		// DBGUPA(stdout, upa, isl_union_pw_aff_get_ctx(upa), "get_dimission_union_pw_aff upa:");
		ub = obatian_union_pw_aff_bound(isl_union_pw_aff_copy(upa), isl_union_set_copy(domain));
		// DBGVAL(stdout, ub, isl_val_get_ctx(ub), "ub:");
		ub = isl_val_add(isl_val_copy(ub), isl_val_copy(one));
		// DBGVAL(stdout, ub, isl_val_get_ctx(ub), "ub:");
		if (i == 0)
		{
			sum_ub = isl_val_copy(ub);
			if (isl_val_le(sum_ub, grid_size))
			{
				// printf("ub <= grid \n");
				dim->result = isl_union_pw_aff_copy(upa);
				dim->inter_dim = get_empty_union_pw_aff(upa);
			}
			else
			{
				dim->inter_dim_valid = 1;
				dim->inter_dim = isl_union_pw_aff_floor(isl_union_pw_aff_scale_down_val(isl_union_pw_aff_copy(upa), isl_val_copy(grid_size)));
				dim->result = isl_union_pw_aff_mod_val(isl_union_pw_aff_copy(upa), isl_val_copy(grid_size));
			}
			// DBGUPA(stdout, dim->result, isl_union_pw_aff_get_ctx(dim->result), "i==0 dim->result:");
		}
		else
		{
			sum_ub = isl_val_mul(sum_ub, isl_val_copy(ub));
			// DBGVAL(stdout, sum_ub, isl_val_get_ctx(sum_ub), "sum_ub:");
			// DBGVAL(stdout, sum_ub, isl_val_get_ctx(sum_ub), "sum_ub:");
			// if (sum_ub <= grid_size)
			if (isl_val_le(sum_ub, grid_size))
			{
				//   printf("ub <= grid \n");
				tmp = isl_union_pw_aff_scale_val(isl_union_pw_aff_copy(upa), isl_val_copy(previous_bound));
				dim->result = isl_union_pw_aff_add(dim->result, isl_union_pw_aff_copy(tmp));
			}
			else
			{
				dim->inter_dim_valid = 1;
		...
		// previous_bound = isl_val_copy(ub);
		previous_bound = isl_val_mul(isl_val_copy(previous_bound), isl_val_copy(ub));
	}
	
	// printf("[Function] get_dimission_union_pw_aff: loop done.");
	if (!dim->inter_dim_valid)
	{
		dim->sum = isl_val_copy(one);
	}
	{
		dim->sum = isl_val_copy(sum_ub);
	}
	return dim;
}
...
__isl_give isl_multi_union_pw_aff *isl_schedule_node_get_prefix_space_multi_union_pw_aff(
	__isl_keep isl_schedule_node *node, __isl_keep isl_multi_val *grid, int include_cluster)
{
	enum isl_schedule_node_type type;
	enum isl_polyacc_loop_type loop_type;
	isl_bool is_cluster;
	...
	isl_ctx *ctx;
	isl_union_set *domain;
	isl_union_pw_aff_list *upa_list_x;
	isl_union_pw_aff_list *upa_list_y;
	isl_union_pw_aff_list *upa_list_z;
	isl_size n_member;
	//   isl_schedule_node *node_copy;
	isl_multi_union_pw_aff *band_mupa;
	isl_union_pw_aff *band_upa;

	domain = isl_schedule_node_get_domain(node);
	ctx = isl_schedule_node_get_ctx(node);

	upa_list_x = isl_union_pw_aff_list_alloc(ctx, 0);
	upa_list_y = isl_union_pw_aff_list_alloc(ctx, 0);
	upa_list_z = isl_union_pw_aff_list_alloc(ctx, 0);

	// obtain original upa_list of different dimension
	while (isl_schedule_node_has_parent(node))
	{
		type = isl_schedule_node_get_type(node);
		if (type == isl_schedule_node_band)
		{
			...
			{
				loop_type = isl_schedule_node_band_member_get_space_time(node, i);
				band_upa = isl_multi_union_pw_aff_get_union_pw_aff(band_mupa, i);
				is_cluster = isl_schedule_node_band_member_get_coincident(node, i);
				// printf("[Function] isl_schedule_node_get_prefix_space_multi_union_pw_aff: is_cluster = %d\n", is_cluster);
				if(include_cluster==1){
					if (loop_type == isl_polyacc_loop_spatial_x)
					{
						upa_list_x = isl_union_pw_aff_list_add(upa_list_x, band_upa);
					}
					if (loop_type == isl_polyacc_loop_spatial_y)
					{
						upa_list_y = isl_union_pw_aff_list_add(upa_list_y, band_upa);
					}
					if (loop_type == isl_polyacc_loop_spatial_z)
					{
						upa_list_z = isl_union_pw_aff_list_add(upa_list_z, band_upa);
					}
				}else{
					if ((loop_type == isl_polyacc_loop_spatial_x) & (!is_cluster))
					{
						upa_list_x = isl_union_pw_aff_list_add(upa_list_x, band_upa);
					}
					if ((loop_type == isl_polyacc_loop_spatial_y) & (!is_cluster))
					{
						upa_list_y = isl_union_pw_aff_list_add(upa_list_y, band_upa);
					}
					if ((loop_type == isl_polyacc_loop_spatial_z) & (!is_cluster))
					{
						upa_list_z = isl_union_pw_aff_list_add(upa_list_z, band_upa);
					}
				}
	...
	// printf("upa list X-dim: \n");
	// isl_union_pw_aff_list_dump(upa_list_x);
	// printf("upa list Y-dim: \n");
	// isl_union_pw_aff_list_dump(upa_list_y);
	// printf("upa list Z-dim: \n");
	// isl_union_pw_aff_list_dump(upa_list_z);

	isl_size x_upa_size = isl_union_pw_aff_list_size(upa_list_x);
	isl_size y_upa_size = isl_union_pw_aff_list_size(upa_list_y);
	isl_size z_upa_size = isl_union_pw_aff_list_size(upa_list_z);

	isl_val_list *grid_list;
	isl_size n_grid;
	isl_union_pw_aff *upwaff;
	// isl_union_pw_aff *pw_aff;
	isl_space *list_space;
	isl_union_pw_aff *inter_dim;
	isl_union_pw_aff_list *upa_list;
	int inter_dim_valid;

	struct intermediate_data *dim = isl_calloc_type(
		ctx, struct intermediate_data);

	grid_list = isl_multi_val_get_list(grid);
	n_grid = isl_val_list_n_val(grid_list);
	list_space = isl_space_set_alloc(ctx, 0, 0);
	upa_list = isl_union_pw_aff_list_alloc(ctx, 0);
	inter_dim_valid = 0;

	if (n_grid >= 1)
	{
		if(x_upa_size > 0){
			// printf("[Function] isl_schedule_node_get_prefix_space_multi_union_pw_aff: n_grid = %d\n", n_grid);
			dim = get_dimission_union_pw_aff(ctx, isl_union_pw_aff_list_copy(upa_list_x), domain, isl_multi_val_get_val(grid, 0));
			// printf("[Function] isl_schedule_node_get_prefix_space_multi_union_pw_aff: get_dimission_union_pw_aff done.");
			if ((!dim->inter_dim_valid) && (!inter_dim_valid))
			{
				upwaff = isl_union_pw_aff_copy(dim->result);
			}
			else
			{
				if (dim->inter_dim_valid && (!inter_dim_valid))
				{
					inter_dim_valid = 1;
					inter_dim = isl_union_pw_aff_copy(dim->inter_dim);
					upwaff = isl_union_pw_aff_copy(dim->result);
				}
				else
				{
					if ((!dim->inter_dim_valid) && (!inter_dim_valid))
					{
						upwaff = isl_union_pw_aff_scale_val(isl_union_pw_aff_copy(inter_dim), dim->sum);
						upwaff = isl_union_pw_aff_add(upwaff, isl_union_pw_aff_copy(dim->result));
						inter_dim = isl_union_pw_aff_copy(upwaff);
					}
					else
					{
						upwaff = isl_union_pw_aff_scale_val(isl_union_pw_aff_copy(inter_dim), dim->sum);
						upwaff = isl_union_pw_aff_add(upwaff, isl_union_pw_aff_copy(dim->result));
						inter_dim = isl_union_pw_aff_add(isl_union_pw_aff_copy(upwaff), isl_union_pw_aff_copy(dim->inter_dim));
					}
				}
			}
			// DBGUPA(stdout, upwaff, isl_union_pw_aff_get_ctx(upwaff), " n_grid == 1 upwaff:");
			upa_list = isl_union_pw_aff_list_add(upa_list, isl_union_pw_aff_copy(upwaff));
			list_space = isl_space_add_dims(list_space, isl_dim_set, 1);
		}
	}
	if (n_grid >= 2)
	{
			if(y_upa_size > 0){
				// printf("[Function] isl_schedule_node_get_prefix_space_multi_union_pw_aff: n_grid = %d\n", n_grid);
				dim = get_dimission_union_pw_aff(ctx, isl_union_pw_aff_list_copy(upa_list_y), domain, isl_multi_val_get_val(grid, 1));
				if ((!dim->inter_dim_valid) && (!inter_dim_valid))
				{
					upwaff = isl_union_pw_aff_copy(dim->result);
				}
				...
				// DBGUPA(stdout, upwaff, isl_union_pw_aff_get_ctx(upwaff), " n_grid == 2 upwaff:");
				upa_list = isl_union_pw_aff_list_add(upa_list, isl_union_pw_aff_copy(upwaff));
				list_space = isl_space_add_dims(list_space, isl_dim_set, 1);
			}else{
	...
	if (n_grid == 3)
	{
				if(z_upa_size > 0){
					// printf("[Function] isl_schedule_node_get_prefix_space_multi_union_pw_aff: n_grid = %d\n", n_grid);
					dim = get_dimission_union_pw_aff(ctx, isl_union_pw_aff_list_copy(upa_list_z), domain, isl_multi_val_get_val(grid, 2));
					if ((!dim->inter_dim_valid) && (!inter_dim_valid))
					{
						upwaff = isl_union_pw_aff_copy(dim->result);
					}
	...
	// isl_union_pw_aff_list_dump(upa_list);
	// printf("[Function] isl_schedule_node_get_prefix_space_multi_union_pw_aff: upa_list size = %d\n", isl_union_pw_aff_list_size(upa_list));
	if (isl_union_pw_aff_list_size(upa_list) <= 0)
	{
		
		// isl_space *no_spa_space;
		// isl_local_space *ls;
		// isl_constraint *c;
		// isl_basic_set *bset;
		// isl_val *grid_val;

		// no_spa_space = isl_space_set_alloc(ctx, 0, n_grid);
		// ls = isl_local_space_from_space(no_spa_space);
		// bset = isl_basic_set_universe(no_spa_space);
		...
		// 	bset = isl_basic_set_add_constraint(isl_basic_set_copy(bset), c);

		// 	grid_val = isl_multi_val_get_val(grid, i);
		// 	// DBGVAL(stdout, grid_val, isl_val_get_ctx(grid_val), "grid_val");   
		// 	grid_val = isl_val_add(isl_val_copy(grid_val), isl_val_int_from_si(ctx, -1));
		// 	c = isl_constraint_alloc_inequality(isl_local_space_copy(ls));
		// 	c = isl_constraint_set_constant_val(c, grid_val);
		// 	c = isl_constraint_set_coefficient_si(c, isl_dim_set, i, -1);
		// 	bset = isl_basic_set_add_constraint(isl_basic_set_copy(bset), c);
		// }

		// // bset = isl_basic_set_project_out(bset, isl_dim_set, 1, 2);
		// printf("[INFO]basic set test\n");
		// isl_basic_set_dump(bset);

		// isl_union_set *uset = isl_union_map_from_basic_map(bset);
		// isl_union_map *umap = isl_union_map_from_domain_and_range(domain, bset);


		// printf("[INFO]current level memory has not spatial affine.\n");
		isl_val *grid_0;
		// isl_space *space;
		isl_pw_aff *pwaff_0;
		isl_set *set_0;
		...
		grid_0 = isl_multi_val_get_val(grid, 0);
		one_hw = isl_val_get_num_si(isl_val_copy(grid_0));

		// printf("one_hw = %d\n", one_hw);
		if (one_hw <= 0)
		{
			printf("The number of hardware units must be greater than or equal to 1.\n");
			return NULL;
		}

		// space = isl_union_set_get_space_set_alloc(ctx, 0, 1);
		set_0 = isl_set_from_union_set(domain);
		pwaff_0 = isl_pw_aff_val_on_domain(set_0, grid_0);
		upa_0 = isl_union_pw_aff_from_pw_aff(pwaff_0);
		mupa = isl_multi_union_pw_aff_from_union_pw_aff(upa_0);
	}
	else
	{
		if (isl_union_pw_aff_list_size(upa_list) > 0)
		{
			// isl_union_pw_aff_list *upa_list_reverse = isl_union_pw_aff_list_reverse(upa_list);
			mupa = isl_multi_union_pw_aff_from_union_pw_aff_list(list_space, upa_list);
		}
	}
	// isl_multi_union_pw_aff_dump(mupa);

	//   isl_val_list_free(grid_list);
	//   isl_union_pw_aff_free(upwaff);
	//   isl_union_pw_aff_free(inter_dim);
	//   isl_union_pw_aff_list_free(upa_list_x);
	//   isl_union_pw_aff_list_free(upa_list_y);
	//   isl_union_pw_aff_list_free(upa_list_z);
	//   intermediate_data_free(dim);

	return mupa;
}

__isl_give isl_union_map *isl_schedule_node_get_prefix_time_union_map(
	__isl_keep isl_schedule_node *node, __isl_keep isl_multi_union_pw_aff *spatial_mupa, __isl_keep isl_mat *dep_mat)
{
	isl_multi_union_pw_aff *mupa = isl_schedule_node_get_prefix_time_multi_union_pw_aff(node, spatial_mupa, dep_mat);
	isl_union_map *time_map = isl_union_map_from_multi_union_pw_aff(mupa);

	return time_map;
}

__isl_give isl_union_map *isl_schedule_node_get_prefix_space_union_map(
	__isl_keep isl_schedule_node *node, __isl_keep isl_multi_val *grid, int include_cluster)
{
	isl_multi_union_pw_aff *mupa = isl_schedule_node_get_prefix_space_multi_union_pw_aff(node, grid, include_cluster);
	isl_union_map *space_map = isl_union_map_from_multi_union_pw_aff(isl_multi_union_pw_aff_copy(mupa));
 ...
 */
__isl_give isl_vec *isl_schedule_node_get_dep_dis_at_node(
	__isl_keep isl_basic_map *dep, __isl_keep isl_schedule_node *band)
{
	if (isl_schedule_node_get_type(band) != isl_schedule_node_band)
		return NULL;

	printf("enter isl_schedule_node_get_dep_dis_at_node\n");
	//   isl_schedule_node_dump(band);
	...
	for (int i = 0; i < band_w; i++)
	{
		...
		// printf("Obtain the schedule for the dest statement\n");
		...
		// printf("Step 1: Extend the scheduling function.\n");
		...
		printf("Step 2: Convert the basic_map into basic_set.\n");
		...
		printf("Step 3: Intersect the scheduling function with the domain.\n");
 ...
 */
__isl_give isl_schedule_node *isl_schedule_node_tile_band_spec(
	__isl_take isl_schedule_node *node, __isl_take isl_multi_val *sizes)
{
	isl_ctx *ctx = isl_schedule_node_get_ctx(node);
	int scale_tile;
